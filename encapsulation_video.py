...
# Usage example: python3 encapsulation_video.py --input greyscaleVideo.mp4
...
def create_video_writer(args, cap):
    """
    Create a video writer only once.
    :return: A video writer
    """
    hasFrame, frame = cap.read()
    outputFile = args.input[:-4] + '_colorized_encapsulation.avi'
...
def save_output(img_bgr_out, vid_writer):
    vid_writer.write((img_bgr_out * 255).astype(np.uint8))


def main():
    # Import video
    cap, args = read_single_Video()
    ...
    # Create vid_writer
    vid_writer = create_video_writer(args, cap)
    ...
    while cv.waitKey(1):
        ...
        # Scale the frame back and merge three channels
        ab_dec_us = resize_as_original_size(frame, ab_dec)
        img_bgr_out = concatenate_channels(img_l, ab_dec_us)

        # Save
        save_output(img_bgr_out, vid_writer)

    vid_writer.release()

    print('Colorized video saved avi file ended with word encapsulation!')
    print('Done !!!')
